...
# Main job spec
#SBATCH --array=[17-30] #[1-27,30]                            # job array (item identifier is %a)
#SBATCH --time=06:00:00                        # walltime
#SBATCH --ntasks=4 #1                              # not quite sure if this is the right way to specify number of processes?
##SBATCH --nodes=4
#SBATCH --ntasks-per-node=4
##SBATCH --cpus-per-task=1
#SBATCH --mem-per-cpu=40G #70G                       # memory per cpu (should end up as memory per task)
...
python3 -m LoSSETT.control.run_lossett_kscale_native "DYAMOND3" "n2560RAL3" "none" 2020 9 ${SLURM_ARRAY_TASK_ID} "12" ${max_r_deg} "none" 200
